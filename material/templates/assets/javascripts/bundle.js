...
(() => {
  ...
  ***************************************************************************** */
  ...
  function fetchScripts() {
    return typeof mermaid === "undefined" || mermaid instanceof Element ? watchScript("https://unpkg.com/mermaid@10.6.1/dist/mermaid.min.js") : of(void 0);
  }
  ...
  function lookup(head) {
    const canonical = getOptionalElement("[rel=canonical]", head);
    ...
    for (const el of getElements(":scope > *", head)) {
      ...
      for (const key of ["href", "src"]) {
        ...
        if (value === null)
          continue;
        const url = new URL(value, canonical == null ? void 0 : canonical.href);
        const ref = el.cloneNode();
  ...
  function setupInstantNavigation({ location$: location$2, viewport$: viewport$2, progress$: progress$2 }) {
    ...
    const response$ = location$2.pipe(
      ...
      skip(1),
      switchMap(
        (url) => request(url, { progress$: progress$2 }).pipe(
          catchError(() => {
    ...
    const document$2 = response$.pipe(
      ...
      switchMap((res) => {
        ...
        for (const selector of [
          ...
          const source2 = getOptionalElement(selector);
          const target2 = getOptionalElement(selector, next);
          if (typeof source2 !== "undefined" && typeof target2 !== "undefined") {
            source2.replaceWith(target2);
          }
        }
        const source = lookup(document.head);
        const target = lookup(next.head);
        for (const [html, el] of target) {
          if (el.getAttribute("rel") === "stylesheet" || el.hasAttribute("src"))
            continue;
          if (source.has(html)) {
            source.delete(html);
          } else {
    ...
    location$2.pipe(
      startWith(getLocation()),
      bufferCount(2, 1),
      filter(([prev, next]) => prev.pathname === next.pathname && prev.hash !== next.hash),
      map(([, next]) => next)
    ).subscribe((url) => {
      var _a2, _b;
